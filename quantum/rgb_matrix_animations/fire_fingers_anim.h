...
bool DIGITAL_RAIN(effect_params_t* params) {
    ...
    const uint8_t drop_ticks           = 128 - (rgb_matrix_config.speed * 0.48);
    const uint8_t pure_green_intensity = 0xd0;
    const uint8_t max_brightness_boost = 0x90;
    const uint8_t max_intensity        = 0xf0;
        ...
        for (uint8_t row = 0; row < MATRIX_ROWS; row++) {
            ...
            if (led_count > 0) {
                if (g_rgb_frame_buffer[row][col] > pure_green_intensity) {
                    ...
                    const uint8_t green = (uint8_t)((uint16_t)max_intensity * g_rgb_frame_buffer[row][col] / pure_green_intensity) + 5;
                    rgb_matrix_set_color(led[0], 5, green, 5);
                }
            }
        }
    }
