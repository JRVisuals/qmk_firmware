...
const uint16_t PROGMEM clv_combo[]   = {KC_C, KC_L, COMBO_END};

combo_t key_combos[COMBO_COUNT] = {
    [MB_SCRATCH] = COMBO_ACTION(copy_combo), [RL_PASTE] = COMBO_ACTION(paste_combo), [CD_HAPPY] = COMBO_ACTION(happy_combo), [QD_LOOK] = COMBO_ACTION(look_combo), [VZ_UNDO] = COMBO_ACTION(undo_combo), [L_BACK] = COMBO_ACTION(back_combo), [SQ_REACT] = COMBO_ACTION(react_combo), [CLV] = COMBO_ACTION(clv_combo),
};
